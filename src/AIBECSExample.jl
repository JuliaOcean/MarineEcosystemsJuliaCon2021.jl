...
Radiocarbon decays with a halflife  of about 5730 yr, which is slightly larger but similar to the overturning timescale of the ocean.
As such, deviations of ¹⁴C away from atmoshperic values serve as a tracer label for the time passed since a water parcel was last in contact with the atmosphere.
...
md"""
## Tracer Equation (continuous)

Mathematically, the the dissolved inorganic ¹⁴C concentration, denoted $R$ (for Radiocarbon), satisfies the following tracer equation:
...
# ╔═╡ 0ecee92d-8fec-4b65-8ba9-d5871e403353
html"""
<h2>Tracer equation (discrete)</h2>
...
md"""
Let's plot a horizontal slice of the age. (There's a Plots.jl recipe for that!)

Just chose a depth!

depth = $(@bind depth Slider(0:50:6000, show_value=true, default=500))
"""

# ╔═╡ d3b759b6-2e71-46b2-b4c7-7dfdda0877cb
md"""
Or look at a meridional slices at a given longitude using the `plotmeridionalslice` plot recipe.

Just chose a longitude!

longitude = $(@bind lon Slider(-180:180, show_value=true, default=-30))°E
"""

# ╔═╡ 6a506c41-41b8-4971-83e4-6faf53a0bd38
md"""
We can look at a a depth profile at a given location as well

Just chose a latitude! (the longitude slider is 2 cells above)

latitude = $(@bind lat Slider(-90:90, show_value=true, default=30))°N
"""

# ╔═╡ e552da86-a408-4b3f-a0b6-eb9e9702ddc5
lonlat = (lon, lat) # a tuple of longitude and latitude sliders
...
plotverticalmean(sum(k*mask for (k,mask) in enumerate(masks)), grd; title="Ocean basins: ATL (1), PAC (2), and IND (3)")
...
plot_options = (; colorbartitle=" \nage", color=:viridis, right_margin=3Plots.mm, clim)

# ╔═╡ bd159016-9a4c-4313-a3d4-259573d19f02
let
	plothorizontalslice(C14age, grd; depth, title="Radiocarbon age at $(depth)m", plot_options...)
	vline!([mod(lon, 360)], lab="", c=:gray)
	hline!([lat], lab="", c=:gray)
end

# ╔═╡ ebce3a87-1fd8-4f63-a7ac-c97476c15323
let
	plotmeridionalslice(C14age, grd; lon, title="Radiocarbon age at $(lon)°E", plot_options...)
	hline!([depth], lab="", c=:gray)
	vline!([lat], lab="", c=:gray)
end

# ╔═╡ 66364ec2-d852-4d02-940f-feafd7deb53d
let
	plotdepthprofile(C14age, grd; lonlat, title="Radiocarbon age at ($(lat)°N, $(lon)°E)", plot_options..., clim=clim)
end

# ╔═╡ f0d1a772-2b1d-4feb-8d52-b9dbf29b3f45
plotzonalaverage(C14age, grd; tit="Zonal average of radiocarbon age", plot_options...)

# ╔═╡ ee1d8881-4dce-45d3-95fa-9bb33819ebe9
plothorizontalaverage(C14age, grd; title="Horizontally averaged radiocarbon age", plot_options..., xlim=(0,Inf))

# ╔═╡ 98508321-fb4b-4076-9c95-0d38f63303b8
let
	p = plot()
	for (color, (m,mask)) in enumerate(pairs(masks))
		plothorizontalaverage!(C14age, grd; mask, title="Basin averaged radiocarbon age", plot_options..., color, lab="$m")
	end
	p
end

# ╔═╡ 8f76c559-ef89-46c0-a792-75a1de96f6e5
plot([plotzonalaverage(C14age, grd; mask, title="$m-ZA", plot_options..., yunit=u"km") for (m,mask) in pairs(masks)]..., layout=(3,1), size=(500,800))
...
[[AIBECS]]
deps = ["Bijectors", "CodecZlib", "DataDeps", "DataFrames", "Dates", "DiffEqBase", "Distances", "Distributions", "Downloads", "FieldMetadata", "Flatten", "ForwardDiff", "ImageFiltering", "Interpolations", "JLD2", "LinearAlgebra", "MetadataArrays", "NCDatasets", "NearestNeighbors", "OceanGrids", "Printf", "RecipesBase", "Reexport", "Shapefile", "SparseArrays", "SuiteSparse", "UnPack", "Unitful", "UnitfulRecipes"]
git-tree-sha1 = "a2ff61fedfc46c464404f392ee7df2f985681af4"
...
git-tree-sha1 = "6b92532432901b073292cf5e2d6a928651d125d5"
...
[[OffsetArrays]]
deps = ["Adapt"]
git-tree-sha1 = "93e9f0b571c1ddaabebe5a6a52e16be53a97fe25"
...
git-tree-sha1 = "28156897ef216fbc1f182d9189fd2b00ec41f510"
